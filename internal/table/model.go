...
// no sort → ASC → DESC → no sort (same cycle for all modes)
func (m Model) toggleSort() (Model, tea.Cmd) {
	...
	if m.sortColumn != currentCol {
		...
		// Same column, was DESC (or any other state) - remove sort entirely
		m.sortColumn = ""
		m.sortDirection = ""
	}
...
func extractSortFromQuery(sql string) (column string, direction string) {
	...
	// Handles optional table alias prefix (e.g., u.name → name)
	orderByRegex := regexp.MustCompile(
		`(?i)\s+ORDER\s+BY\s+(?:\w+\.)?(\w+)(?:\s+(ASC|DESC))?`,
	)
	...
	if len(lastOuterMatch) >= 2 {
		column = strings.ToLower(lastOuterMatch[1])
